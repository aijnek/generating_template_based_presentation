...
**🚨 レイアウトバリエーションの必須ルール（単調さ回避）:**

1. **同じレイアウトを2回続けて使用してはいけない**
   - 例: Layout 1 → Layout 1 → ... は禁止
   - 必ず間に別のレイアウトを挟む

2. **同一レイアウトの再使用は最低2スライド以上間隔を空ける**
   - 良い例: Layout 1 → Layout 2 → Layout 3 → Layout 1
   - 悪い例: Layout 1 → Layout 2 → Layout 1 ← 間隔が1スライドのみ

3. **レイアウト選択の優先順位:**
   - 第1優先: コンテンツに最適なレイアウト
   - 第2優先: 直前と異なるレイアウト（必須）
   - 第3優先: 過去2スライドで使用していないレイアウト

4. **積極的にバリエーションを活用する:**
   - 同じ「タイトル+本文」でも、"Title and Content" と "Two Content" を交互に使う
   - 比較や対比の際は "Two Content", "Comparison" などの2列レイアウトを選ぶ
   - 図表が多い場合は "Picture with Caption", "Content with Caption" などを活用する
   - セクション区切りでは "Section Header" や "Blank" を挟んでリズムを作る

**割当の実行手順:**

1. Phase 2の各スライド内容を1つずつ確認する
2. コンテンツに適したレイアウト候補を2〜3個リストアップする
3. **直前のスライドで使用したレイアウトは候補から除外する（必須）**
4. **過去2スライドで使用したレイアウトは可能な限り避ける**
5. 残った候補から最も適切なレイアウトを選択する
6. 割当結果を確認し、同じレイアウトが連続していないかチェックする
7. 連続している場合は、どちらかのスライドのレイアウトを変更する

**出力例:**
```
Slide 0: Layout 0 ("Title Slide") → 表紙
Slide 1: Layout 1 ("Title and Content") → 背景・課題説明
Slide 2: Layout 2 ("Two Content") → 現状と目標の比較（Layout 1を避けてバリエーション）
Slide 3: Layout 6 ("Section Header") → セクション区切り（リズム変化のため）
Slide 4: Layout 1 ("Title and Content") → 提案内容（2スライド空いたので再使用可）
Slide 5: Layout 8 ("Picture with Caption") → 実装例の図解（Layout 1を避ける）
Slide 6: Layout 2 ("Two Content") → メリット・デメリット比較
...
```

**🔍 割当後の検証チェックリスト:**
- [ ] 同じレイアウトが連続していない（0箇所であること）
- [ ] 各レイアウトの再使用間隔が2スライド以上空いている
- [ ] テンプレートの全レイアウトを可能な限り活用している（偏りがない）
- [ ] コンテンツの種類とレイアウトの特性が一致している
